...
3.  **テスト作成 & メンテナンス (Create/Maintain Tests)**
    *   **【重要】** 実装前に、期待する動作を検証するためのテストコードを作成してください。
    *   **カバレッジ要件**:
        *   **正常系 (Happy Path)**: 基本的な機能が仕様通り動くこと。
        *   **異常系 (Error Cases)**: 無効な入力やエラー条件に対して、システムが適切に振る舞うこと（クラッシュせず、正しいエラーコードを返す等）。
    *   **継続的なメンテナンス**:
        *   既存機能に変更を加えた場合、必ず関連するテストコードも修正・追加してください。
        *   「テストコードもプロダクトコードの一部である」という意識を持ち、常に最新の状態に保ってください。
